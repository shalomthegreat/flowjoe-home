...
export default function UseCases() {
  return (
    <>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 my-24">
        {cases.map((c) => (
          <div key={c.title} className="bg-cream-soft border border-latte rounded-md p-6">
            <div className="h-28 bg-sand rounded-xl mb-4 flex items-center justify-center">
              <span className="text-5xl">{c.emoji}</span>
            </div>
            <h3 className="text-2xl font-display font-semibold text-espresso">{c.title}</h3>
